...
class SO2(Group):
    ...
    def inverse(self, element: float) -> float:  # 逆元
        r"""
    ...
    def combine(self, e1: float, e2: float) -> float:  # 群乘法
        r"""
    ...
    def equal(self, e1: float, e2: float) -> bool:  # 群元是否相等
        r"""
    ...
    def is_element(self, element: float) -> bool:  # 判断是否群元
    ...
    def subgroup(self, id: int) -> Tuple[Group, Callable, Callable]:  # 子群$C_M$,$M$阶循环群
        r"""
        ...
        if id not in self._subgroups:
            ...
            child_mapping = lambda e, order=order: None if not utils.cycle_isclose(e, 0., 2. * np.pi / order) else int(
                round(e * order / (2. * np.pi)))

            self._subgroups[id] = sg, parent_mapping, child_mapping

        return self._subgroups[id]

    def _restrict_irrep(self, irrep: str, id: int) -> Tuple[np.matrix, List[str]]:  # 限制表示
        r"""
        ...
        change_of_basis = None  # 清空
        irreps = []  # 清空

        f = irr.attributes["frequency"] % order   # 余数
